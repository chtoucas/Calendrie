...
public partial class Egyptian13Calendar // Math
{
    /// <summary>
    ...
    [Pure]
    internal Egyptian13Date AddYears(int y, int m, int d, int years)
    {
        var sch = Schema;
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal Egyptian13Date AddYears(int y, int m, int d, int years, out int roundoff)
    {
        var sch = Schema;
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal Egyptian13Date AddMonths(int y, int m, int d, int months)
    {
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal Egyptian13Date AddMonths(int y, int m, int d, int months, out int roundoff)
    {
        // Exact addition of months to a calendar month.
        int newM = 1 + MathZ.Modulo(
            checked(m - 1 + months), Egyptian13Schema.MonthsPerYear, out int years);
...
#region Egyptian13Clock

/// <summary>
/// Represents a clock for the Egyptian calendar.
/// <para>This class cannot be inherited.</para>
/// </summary>
public sealed partial class Egyptian13Clock : IClock
{
    /// <summary>
    /// Initializes a new instance of the <see cref="Egyptian13Clock"/> class.
    /// </summary>
    /// <exception cref="ArgumentNullException"><paramref name="clock"/> is
    /// <see langword="null"/>.</exception>
    public Egyptian13Clock(IClock clock)
    {
    ...
    /// <summary>
    /// Gets an instance of the <see cref="Egyptian13Clock"/> class for the
    /// system clock using the current time zone setting on this machine.
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    public static Egyptian13Clock Local { get; } = new(LocalSystemClock.Instance);
    ...
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    public static Egyptian13Clock Utc { get; } = new(UtcSystemClock.Instance);
    ...
    /// <summary>
    /// Obtains a <see cref="Egyptian13Date"/> value representing the current date.
    /// </summary>
    [Pure]
    public Egyptian13Date GetCurrentDate() => Egyptian13Date.FromAbsoluteDate(Clock.Today());

    /// <summary>
    /// Obtains a <see cref="Egyptian13Month"/> value representing the current month.
    /// </summary>
    [Pure]
    public Egyptian13Month GetCurrentMonth() => Egyptian13Month.FromDate(GetCurrentDate());

    /// <summary>
    /// Obtains a <see cref="Egyptian13Year"/> value representing the current year.
    /// </summary>
    [Pure]
    public Egyptian13Year GetCurrentYear() => Egyptian13Year.FromDate(GetCurrentDate());
}
...
public partial struct Egyptian13Date // Find a close by day of the week
{
    /// <inheritdoc />
    [Pure]
...
public partial struct Egyptian13Date // Non-standard math ops
{
    ...
    /// range of supported dates.</exception>
    [Pure]
    public Egyptian13Date PlusYears(int years, out int roundoff)
    {
        var chr = Calendar;
        chr.Schema.GetDateParts(_daysSinceEpoch, out int y, out int m, out int d);
    ...
    /// range of supported dates.</exception>
    [Pure]
    public Egyptian13Date PlusMonths(int months, out int roundoff)
    {
        var sch = Calendar.Schema;
        sch.GetDateParts(_daysSinceEpoch, out int y, out int m, out int d);
...
public partial struct Egyptian13Month // Non-standard math ops
{
    ...
    [Pure]
    Egyptian13Month IMonthBase<Egyptian13Month>.PlusYears(int years, out int roundoff)
    {
        roundoff = 0;
