...
public partial class PositivistCalendar // Math
{
    /// <summary>
    ...
    [Pure]
    internal PositivistDate AddYears(int y, int m, int d, int years)
    {
        var sch = Schema;
        ...
        return PositivistDate.UnsafeCreate(daysSinceEpoch);
    }
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal PositivistDate AddYears(int y, int m, int d, int years, out int roundoff)
    {
        var sch = Schema;
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal PositivistDate AddMonths(int y, int m, int d, int months)
    {
        // Exact addition of months to a calendar month.
        int newM = 1 + MathZ.Modulo(
            checked(m - 1 + months), PositivistSchema.MonthsPerYear, out int years);
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal PositivistDate AddMonths(int y, int m, int d, int months, out int roundoff)
    {
        // Exact addition of months to a calendar month.
        int newM = 1 + MathZ.Modulo(
            checked(m - 1 + months), PositivistSchema.MonthsPerYear, out int years);
...
#region PositivistClock

/// <summary>
/// Represents a clock for the Positivist calendar.
/// <para>This class cannot be inherited.</para>
/// </summary>
public sealed partial class PositivistClock : IClock
{
    /// <summary>
    /// Initializes a new instance of the <see cref="PositivistClock"/> class.
    /// </summary>
    /// <exception cref="ArgumentNullException"><paramref name="clock"/> is
    /// <see langword="null"/>.</exception>
    public PositivistClock(IClock clock)
    {
    ...
    /// <summary>
    /// Gets an instance of the <see cref="PositivistClock"/> class for the
    /// system clock using the current time zone setting on this machine.
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    public static PositivistClock Local { get; } = new(LocalSystemClock.Instance);
    ...
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    public static PositivistClock Utc { get; } = new(UtcSystemClock.Instance);
    ...
    /// <summary>
    /// Obtains a <see cref="PositivistDate"/> value representing the current date.
    /// </summary>
    [Pure]
    public PositivistDate GetCurrentDate() => PositivistDate.FromAbsoluteDate(Clock.Today());

    /// <summary>
    /// Obtains a <see cref="PositivistMonth"/> value representing the current month.
    /// </summary>
    [Pure]
    public PositivistMonth GetCurrentMonth() => PositivistMonth.FromDate(GetCurrentDate());

    /// <summary>
    /// Obtains a <see cref="PositivistYear"/> value representing the current year.
    /// </summary>
    [Pure]
    public PositivistYear GetCurrentYear() => PositivistYear.FromDate(GetCurrentDate());
}
...
public partial struct PositivistDate // Preamble
{
    ...
    /// </summary>
    private PositivistDate(int daysSinceEpoch)
    {
...
public partial struct PositivistDate // Factories
{
    ...
    /// <summary>
    /// Attempts to create a new instance of the <see cref="PositivistDate"/>
    ...
    /// <summary>
    /// Creates a new instance of the <see cref="PositivistDate"/> struct
    /// from the specified count of consecutive days since the epoch.
    /// <para>This method does NOT validate its parameter.</para>
    /// </summary>
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static PositivistDate UnsafeCreate(int daysSinceEpoch) => new(daysSinceEpoch);
...
{
    /// <summary>
    /// Defines an implicit conversion of a <see cref="PositivistDate"/> value
    ...
    public static implicit operator DayNumber(PositivistDate date) => date.DayNumber;
    ...
    [Pure]
    public static PositivistDate FromAbsoluteDate(DayNumber dayNumber)
    {
    ...
    [Pure]
    static PositivistDate IAbsoluteDate<PositivistDate>.FromDayNumber(DayNumber dayNumber) =>
...
public partial struct PositivistDate // Find a close by day of the week
{
    /// <inheritdoc />
    [Pure]
...
public partial struct PositivistDate // Standard math ops
{
    ...
    /// </exception>
    [Pure]
    public PositivistDate PlusWeeks(int weeks) => PlusDays(DaysInWeek * weeks);
...
public partial struct PositivistDate // Non-standard math ops
{
    ...
    /// range of supported dates.</exception>
    [Pure]
    public PositivistDate PlusYears(int years, out int roundoff)
    {
        var chr = Calendar;
        chr.Schema.GetDateParts(_daysSinceEpoch, out int y, out int m, out int d);
    ...
    /// range of supported dates.</exception>
    [Pure]
    public PositivistDate PlusMonths(int months, out int roundoff)
    {
        var sch = Calendar.Schema;
        sch.GetDateParts(_daysSinceEpoch, out int y, out int m, out int d);
    ...
    public int CountMonthsSince(PositivistDate other)
    {
        ...
        int months = checked(PositivistSchema.MonthsPerYear * (y - y0) + m - m0);
...
/// <summary>
/// Represents the Positivist month.
/// <para><i>All</i> months within the range [1..9999] of years are supported.
/// </para>
/// <para><see cref="PositivistMonth"/> is an immutable struct.</para>
/// </summary>
public readonly partial struct PositivistMonth :
    IMonth<PositivistMonth>,
    IUnsafeFactory<PositivistMonth>,
    // A month viewed as a finite sequence of days
    IDaySegment<PositivistDate>,
    ISetMembership<PositivistDate>,
    // Arithmetic
    ISubtractionOperators<PositivistMonth, PositivistMonth, int>
{ }

public partial struct PositivistMonth // Preamble
{
    ...
    public PositivistMonth(int year, int month)
    {
        // The calendar being regular, no need to use the Scope:
        // > PositivistCalendar.Instance.Scope.ValidateYearMonth(year, month);
        if (year < StandardScope.MinYear || year > StandardScope.MaxYear)
            ThrowHelpers.ThrowYearOutOfRange(year);
        if (month < 1 || month > PositivistSchema.MonthsPerYear)
    ...
    /// </summary>
    private PositivistMonth(int monthsSinceEpoch)
    {
        _monthsSinceEpoch = monthsSinceEpoch;
    }

    /// <summary>
    /// Gets the smallest possible value of <see cref="PositivistMonth"/>.
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    /// <returns>The earliest supported month.</returns>
    //
    // MinValue = new(0) = new() = default(PositivistMonth)
    public static PositivistMonth MinValue { get; }

    /// <summary>
    /// Gets the largest possible value of <see cref="PositivistMonth"/>.
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    /// <returns>The latest supported month.</returns>
    public static PositivistMonth MaxValue { get; } = new(MaxMonthsSinceEpoch);
    ...
    public int Year =>
        ...
        1 + _monthsSinceEpoch / PositivistSchema.MonthsPerYear;
    ...
    {
        var (y, m) = this;
        return FormattableString.Invariant($"{m:D2}/{y:D4} ({PositivistCalendar.DisplayName})");
    }
    ...
    public void Deconstruct(out int year, out int month)
    {
        ...
        // NB: both dividend and divisor are >= 0.
        year = 1 + MathN.Divide(_monthsSinceEpoch, PositivistSchema.MonthsPerYear, out int m0);
        month = 1 + m0;
    }
}

public partial struct PositivistMonth // Factories
{
    /// <inheritdoc />
    [Pure]
    public static PositivistMonth Create(int year, int month) => new(year, month);

    /// <summary>
    /// Attempts to create a new instance of the <see cref="PositivistMonth"/>
    /// struct from the specified month components.
    /// </summary>
    [Pure]
    public static PositivistMonth? TryCreate(int year, int month)
    {
        // The calendar being regular, no need to use the PreValidator.
        if (year < StandardScope.MinYear || year > StandardScope.MaxYear
            || month < 1 || month > PositivistSchema.MonthsPerYear)
        {
            return null;
        }
    ...
    // Explicit implementation: PositivistMonth being a value type, better
    // to use the other TryCreate().
    [Pure]
    static bool IMonth<PositivistMonth>.TryCreate(int year, int month, out PositivistMonth result)
    {
    ...
    /// </summary>
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static PositivistMonth UnsafeCreate(int year, int month)
    {
    ...
    /// </summary>
    [Pure, MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static PositivistMonth UnsafeCreate(int monthsSinceEpoch) => new(monthsSinceEpoch);

    [Pure]
    static PositivistMonth IUnsafeFactory<PositivistMonth>.UnsafeCreate(int monthsSinceEpoch) =>
...
public partial struct PositivistMonth // Counting
{
    ...
    /// <inheritdoc />
    [Pure]
    public int CountRemainingMonthsInYear() => PositivistSchema.MonthsPerYear - Month;
...
public partial struct PositivistMonth // Adjustments
{
    /// <inheritdoc />
    [Pure]
    public PositivistMonth WithYear(int newYear)
    {
        int m = Month;
    ...
    /// <inheritdoc />
    [Pure]
    public PositivistMonth WithMonth(int newMonth)
    {
        int y = Year;
        ...
        if (newMonth < 1 || newMonth > PositivistSchema.MonthsPerYear)
...
public partial struct PositivistMonth // IDaySegment
{
    /// <summary>
    /// Gets the the start of the current month instance.
    /// </summary>
    public PositivistDate MinDay
    {
        get
        {
            var (y, m) = this;
    ...
    public Range<PositivistDate> ToRange()
    {
        ...
        return Range.StartingAt(PositivistDate.UnsafeCreate(startOfMonth), daysInMonth);
    }
    ...
    /// </summary>
    [Pure]
    public IEnumerable<PositivistDate> ToEnumerable()
    {
        var (y, m) = this;
        var sch = Calendar.Schema;
        int startOfMonth = sch.CountDaysSinceEpoch(y, m, 1);
        ...
        return from daysSinceEpoch
               ...
               select PositivistDate.UnsafeCreate(daysSinceEpoch);
    }

    [Pure]
    IEnumerable<PositivistDate> IDaySegment<PositivistDate>.EnumerateDays() => ToEnumerable();
    ...
    /// </summary>
    [Pure]
    public bool Contains(PositivistDate date)
    {
        var (y, m) = this;
    ...
    [Pure]
    public PositivistDate GetDayOfMonth(int dayOfMonth)
    {
        var (y, m) = this;
        var chr = Calendar;
...
public partial struct PositivistMonth // IEquatable
{
    /// <inheritdoc />
    public static bool operator ==(PositivistMonth left, PositivistMonth right) =>
    ...
    /// <inheritdoc />
    [Pure]
    public bool Equals(PositivistMonth other) => _monthsSinceEpoch == other._monthsSinceEpoch;

    /// <inheritdoc />
    [Pure]
    public override bool Equals([NotNullWhen(true)] object? obj) =>
        obj is PositivistMonth month && Equals(month);
...
public partial struct PositivistMonth // IComparable
{
    /// <summary>
    ...
    /// <inheritdoc />
    [Pure]
    public static PositivistMonth Min(PositivistMonth x, PositivistMonth y) => x < y ? x : y;

    /// <inheritdoc />
    [Pure]
    public static PositivistMonth Max(PositivistMonth x, PositivistMonth y) => x > y ? x : y;

    /// <inheritdoc />
    [Pure]
    public int CompareTo(PositivistMonth other) => _monthsSinceEpoch.CompareTo(other._monthsSinceEpoch);

    [Pure]
    int IComparable.CompareTo(object? obj) =>
        obj is null ? 1
        : obj is PositivistMonth month ? CompareTo(month)
        : ThrowHelpers.ThrowNonComparable(typeof(PositivistMonth), obj);
}

public partial struct PositivistMonth // Standard math ops
{
    /// <summary>
    ...
    public static int operator -(PositivistMonth left, PositivistMonth right) => left.CountMonthsSince(right);
    ...
    public static PositivistMonth operator +(PositivistMonth value, int months) => value.PlusMonths(months);
    ...
    public static PositivistMonth operator ++(PositivistMonth value) => value.NextMonth();
    ...
    public static PositivistMonth operator --(PositivistMonth value) => value.PreviousMonth();

    /// <summary>
    /// Counts the number of whole months elapsed since the specified month.
    /// </summary>
    [Pure]
    public int CountMonthsSince(PositivistMonth other) =>
        // No need to use a checked context here. Indeed, the absolute value of
    ...
    /// </exception>
    [Pure]
    public PositivistMonth PlusMonths(int months)
    {
    ...
    /// latest supported month.</exception>
    [Pure]
    public PositivistMonth NextMonth()
    {
        if (_monthsSinceEpoch == MaxMonthsSinceEpoch) ThrowHelpers.ThrowMonthOverflow();
        return new PositivistMonth(_monthsSinceEpoch + 1);
    }
    ...
    /// earliest supported month.</exception>
    [Pure]
    public PositivistMonth PreviousMonth()
    {
...
public partial struct PositivistMonth // Non-standard math ops
{
    /// <summary>
    ...
    [Pure]
    public PositivistMonth PlusYears(int years)
    {
        var (y, m) = this;
        // Exact addition of years to a calendar year.
        int newY = checked(y + years);
    ...
    [Pure]
    PositivistMonth IMonthBase<PositivistMonth>.PlusYears(int years, out int roundoff)
    {
        roundoff = 0;
    ...
    /// </summary>
    [Pure]
    public int CountYearsSince(PositivistMonth other)
    {
        // Exact difference between two calendar years.
...
#region PositivistYear

/// <summary>
/// Represents the Positivist year.
/// <para><i>All</i> years within the range [1..9999] of years are supported.
/// </para>
/// <para><see cref="PositivistYear"/> is an immutable struct.</para>
/// </summary>
public readonly partial struct PositivistYear :
    IYear<PositivistYear>,
    // A year viewed as a finite sequence of months
    IMonthSegment<PositivistMonth>,
    ISetMembership<PositivistMonth>,
    // A year viewed as a finite sequence of days
    IDaySegment<PositivistDate>,
    ISetMembership<PositivistDate>,
    // Arithmetic
    ISubtractionOperators<PositivistYear, PositivistYear, int>
{ }

public partial struct PositivistYear // Preamble
{
    ...
    public PositivistYear(int year)
    {
        if (year < StandardScope.MinYear || year > StandardScope.MaxYear)
            ThrowHelpers.ThrowYearOutOfRange(year);
    ...
    /// <para>This method does NOT validate its parameter.</para>
    /// </summary>
    private PositivistYear(ushort yearsSinceEpoch)
    {
        _yearsSinceEpoch = yearsSinceEpoch;
    }

    /// <summary>
    /// Gets the smallest possible value of <see cref="PositivistYear"/>.
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    /// <returns>The earliest supported year.</returns>
    //
    // MinValue = new(1) = new() = default(PositivistYear)
    public static PositivistYear MinValue { get; }

    /// <summary>
    /// Gets the largest possible value of <see cref="PositivistYear"/>.
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    /// <returns>The latest supported year.</returns>
    public static PositivistYear MaxValue { get; } = new((ushort)MaxYearsSinceEpoch);
    ...
    [Pure]
    public override string ToString() =>
        FormattableString.Invariant($"{Year:D4} ({PositivistCalendar.DisplayName})");
}

public partial struct PositivistYear // Factories
{
    /// <inheritdoc />
    [Pure]
    public static PositivistYear Create(int year) => new(year);

    /// <summary>
    /// Attempts to create a new instance of the <see cref="PositivistYear"/>
    /// struct from the specified year.
    /// </summary>
    [Pure]
    public static PositivistYear? TryCreate(int year)
    {
        bool ok = year >= StandardScope.MinYear && year <= StandardScope.MaxYear;
        return ok ? UnsafeCreate(year) : null;
    }

    // Explicit implementation: PositivistYear being a value type, better
    // to use the other TryCreate().
    [Pure]
    static bool IYear<PositivistYear>.TryCreate(int year, out PositivistYear result)
    {
...
public partial struct PositivistYear // IMonthSegment
{
    ...
    /// <inheritdoc />
    public PositivistMonth MinMonth => PositivistMonth.UnsafeCreate(Year, 1);

    /// <inheritdoc />
    public PositivistMonth MaxMonth => PositivistMonth.UnsafeCreate(Year, MonthCount);

    /// <inheritdoc />
    [Pure]
    int IMonthSegment<PositivistMonth>.CountMonths() => MonthCount;
    ...
    /// <inheritdoc />
    [Pure]
    public IEnumerable<PositivistMonth> EnumerateMonths()
    {
        int startOfYear = PositivistMonth.UnsafeCreate(Year, 1).MonthsSinceEpoch;

        return from monthsSinceEpoch
               in Enumerable.Range(startOfYear, MonthCount)
               select PositivistMonth.UnsafeCreate(monthsSinceEpoch);
    }
    ...
    /// </summary>
    [Pure]
    public bool Contains(PositivistMonth month) => month.Year == Year;
    ...
    /// is outside the range of valid values.</exception>
    [Pure]
    public PositivistMonth GetMonthOfYear(int month)
    {
        ...
        return PositivistMonth.UnsafeCreate(Year, month);
    }
}

public partial struct PositivistYear // IDaySegment
{
    /// <summary>
    ...
    public Range<PositivistDate> ToDayRange()
    {
        ...
        return Range.StartingAt(PositivistDate.UnsafeCreate(startOfYear), daysInYear);
    }

    /// <inheritdoc />
    [Pure]
    public IEnumerable<PositivistDate> EnumerateDays()
    {
        var sch = Calendar.Schema;
        int startOfYear = sch.CountDaysSinceEpoch(Year, 1);
        int daysInYear = sch.CountDaysInYear(Year);
    ...
    /// </summary>
    [Pure]
    public bool Contains(PositivistDate date) => date.Year == Year;
    ...
    /// is outside the range of valid values.</exception>
    [Pure]
    public PositivistDate GetDayOfYear(int dayOfYear)
    {
        var chr = Calendar;
...
public partial struct PositivistYear // IEquatable
{
    /// <inheritdoc />
    public static bool operator ==(PositivistYear left, PositivistYear right) =>
    ...
    /// <inheritdoc />
    [Pure]
    public bool Equals(PositivistYear other) => _yearsSinceEpoch == other._yearsSinceEpoch;

    /// <inheritdoc />
    [Pure]
    public override bool Equals([NotNullWhen(true)] object? obj) =>
        obj is PositivistYear year && Equals(year);
...
public partial struct PositivistYear // IComparable
{
    /// <summary>
    /// Compares the two specified instances to see if the left one is strictly
    /// earlier than the right one.
    ...
    /// <inheritdoc />
    [Pure]
    public static PositivistYear Min(PositivistYear x, PositivistYear y) => x < y ? x : y;

    /// <inheritdoc />
    [Pure]
    public static PositivistYear Max(PositivistYear x, PositivistYear y) => x > y ? x : y;

    /// <inheritdoc />
    [Pure]
    public int CompareTo(PositivistYear other) =>
        _yearsSinceEpoch.CompareTo(other._yearsSinceEpoch);

    [Pure]
    int IComparable.CompareTo(object? obj) =>
        obj is null ? 1
        : obj is PositivistYear year ? CompareTo(year)
        : ThrowHelpers.ThrowNonComparable(typeof(PositivistYear), obj);
}

public partial struct PositivistYear // Math ops
{
    /// <summary>
    ...
    public static int operator -(PositivistYear left, PositivistYear right) => left.CountYearsSince(right);
    ...
    public static PositivistYear operator +(PositivistYear value, int years) => value.PlusYears(years);
    ...
    public static PositivistYear operator ++(PositivistYear value) => value.NextYear();
    ...
    public static PositivistYear operator --(PositivistYear value) => value.PreviousYear();

    /// <summary>
    /// Counts the number of whole years elapsed since the specified year.
    /// </summary>
    [Pure]
    public int CountYearsSince(PositivistYear other) =>
        // No need to use a checked context here. Indeed, the absolute value of
    ...
    /// </exception>
    [Pure]
    public PositivistYear PlusYears(int years)
    {
        ...
        return new PositivistYear((ushort)yearsSinceEpoch);
    }
    ...
    /// latest supported year.</exception>
    [Pure]
    public PositivistYear NextYear()
    {
        if (_yearsSinceEpoch == MaxYearsSinceEpoch) ThrowHelpers.ThrowYearOverflow();
        return new PositivistYear((ushort)(_yearsSinceEpoch + 1));
    }
    ...
    /// earliest supported year.</exception>
    [Pure]
    public PositivistYear PreviousYear()
    {
