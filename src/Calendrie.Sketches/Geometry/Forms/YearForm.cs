...
/// </summary>
/// <remarks>
/// <para>To build such a form, we must in general change the origin of years
/// (the epoch) such that the exceptionnal year becomes the last one.</para>
/// </remarks>
    ...
    : CalendricalForm(DaysPerCycle, YearsPerCycle, Remainder)
{
    ...
    public YearForm Normalize()
    {
        ...
        if ((m0, d0) != (1, 1)) throw new InvalidOperationException();
    ...
    /// <summary>
    /// Counts the number of consecutive days from <see cref="CalendricalForm.Origin"/>
    /// to the first day of the specified year.
    /// </summary>
    [Pure]
