...
module Regular12Case =
    ...
    [<Theory; MemberData(nameof(startOfYearMonthsSinceEpochData))>]
    let ``GetStartOfYear()`` (x: YearMonthsSinceEpoch) =
        MonthsCalculator.Regular12.GetStartOfYear(x.Year) === x.MonthsSinceEpoch

    [<Theory; MemberData(nameof(endOfYearMonthsSinceEpochData))>]
    let ``GetEndOfYear()`` (x: YearMonthsSinceEpoch) =
        MonthsCalculator.Regular12.GetEndOfYear(x.Year) === x.MonthsSinceEpoch
...
module Regular13Case =
    ...
    [<Theory; MemberData(nameof(startOfYearMonthsSinceEpochData))>]
    let ``GetStartOfYear()`` (x: YearMonthsSinceEpoch) =
        MonthsCalculator.Regular13.GetStartOfYear(x.Year) === x.MonthsSinceEpoch

    [<Theory; MemberData(nameof(endOfYearMonthsSinceEpochData))>]
    let ``GetEndOfYear()`` (x: YearMonthsSinceEpoch) =
        MonthsCalculator.Regular13.GetEndOfYear(x.Year) === x.MonthsSinceEpoch
