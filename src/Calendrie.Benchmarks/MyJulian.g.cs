...
public partial class MyJulianCalendar // Math
{
    /// <summary>
    ...
    [Pure]
    internal MyJulianDate AddYears(int y, int m, int d, int years)
    {
        var sch = Schema;
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal MyJulianDate AddYears(int y, int m, int d, int years, out int roundoff)
    {
        var sch = Schema;
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal MyJulianDate AddMonths(int y, int m, int d, int months)
    {
        // Exact addition of months to a calendar month.
        int newM = 1 + MathZ.Modulo(
            checked(m - 1 + months), JulianSchema.MonthsPerYear, out int years);
    ...
    /// range of supported dates.</exception>
    [Pure]
    internal MyJulianDate AddMonths(int y, int m, int d, int months, out int roundoff)
    {
        // Exact addition of months to a calendar month.
        int newM = 1 + MathZ.Modulo(
            checked(m - 1 + months), JulianSchema.MonthsPerYear, out int years);
...
#region MyJulianClock

/// <summary>
/// Represents a clock for the Plain Standard Julian calendar.
/// <para>This class cannot be inherited.</para>
/// </summary>
public sealed partial class MyJulianClock : IClock
{
    /// <summary>
    /// Initializes a new instance of the <see cref="MyJulianClock"/> class.
    /// </summary>
    /// <exception cref="ArgumentNullException"><paramref name="clock"/> is
    /// <see langword="null"/>.</exception>
    public MyJulianClock(IClock clock)
    {
    ...
    /// <summary>
    /// Gets an instance of the <see cref="MyJulianClock"/> class for the
    /// system clock using the current time zone setting on this machine.
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    public static MyJulianClock Local { get; } = new(LocalSystemClock.Instance);
    ...
    /// <para>This static property is thread-safe.</para>
    /// </summary>
    public static MyJulianClock Utc { get; } = new(UtcSystemClock.Instance);
    ...
    /// <summary>
    /// Obtains a <see cref="MyJulianDate"/> value representing the current date.
    /// </summary>
    [Pure]
    public MyJulianDate GetCurrentDate() => MyJulianDate.FromAbsoluteDate(Clock.Today());

    /// <summary>
    /// Obtains a <see cref="MyJulianMonth"/> value representing the current month.
    /// </summary>
    [Pure]
    public MyJulianMonth GetCurrentMonth() => MyJulianMonth.FromDate(GetCurrentDate());

    /// <summary>
    /// Obtains a <see cref="MyJulianYear"/> value representing the current year.
    /// </summary>
    [Pure]
    public MyJulianYear GetCurrentYear() => MyJulianYear.FromDate(GetCurrentDate());
}
...
public partial struct MyJulianDate // Factories
{
    /// <inheritdoc />
    [Pure]
...
public partial struct MyJulianDate // Conversions
{
    /// <summary>
    /// Defines an implicit conversion of a <see cref="MyJulianDate"/> value
    ...
    public static implicit operator DayNumber(MyJulianDate date) => date.DayNumber;
    ...
    [Pure]
    public static MyJulianDate FromAbsoluteDate(DayNumber dayNumber)
    {
    ...
    [Pure]
    static MyJulianDate IAbsoluteDate<MyJulianDate>.FromDayNumber(DayNumber dayNumber) =>
...
public partial struct MyJulianDate // Non-standard math ops
{
    ...
    /// range of supported dates.</exception>
    [Pure]
    public MyJulianDate PlusYears(int years, out int roundoff)
    {
        var chr = Calendar;
        chr.Schema.GetDateParts(_daysSinceEpoch, out int y, out int m, out int d);
    ...
    /// range of supported dates.</exception>
    [Pure]
    public MyJulianDate PlusMonths(int months, out int roundoff)
    {
        var sch = Calendar.Schema;
        sch.GetDateParts(_daysSinceEpoch, out int y, out int m, out int d);
...
public partial struct MyJulianMonth // IDaySegment
{
    /// <summary>
    ...
    [Pure]
    IEnumerable<MyJulianDate> IDaySegment<MyJulianDate>.EnumerateDays() => ToEnumerable();
...
public partial struct MyJulianYear // IDaySegment
{
    /// <summary>
